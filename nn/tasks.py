...
from ultralytics.nn.modules.fcm import FCM
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if len(args) == 3:
                dim, qk_dim, mlp_dim = args
                heads = 4
            elif len(args) == 4:
                dim, qk_dim, mlp_dim, heads = args
            else:
                raise ValueError(f"LRSA expects 3 or 4 args but got {len(args)}")
            args = [dim, qk_dim, mlp_dim, heads]
